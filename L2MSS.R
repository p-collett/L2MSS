...
# Read in data
fa_data <- read.csv("L2MSS_data.csv", header = TRUE, sep = ",")

# Split data set for EFA and CFA
N <- nrow(fa_data)
indices <- 1:N
indices_EFA <- sample(indices, floor(0.5 * N))
indices_CFA <- indices[!(indices %in% indices_EFA)]
# EFA data
data_EFA <- fa_data[indices_EFA, ]
# CFA data
data_CFA <- fa_data[indices_CFA, ]
...
group6 <- c(91:109,120:122)

## EFA Function ##

run_efa <- function(data, cols, nfactors = NULL, rotation = "geominT") {
  ...
  # Run parallel analysis for factor determination
  cat("=== Parallel Analysis ===\n")
  parallel_result <- fa.parallel(data_subset, plot = TRUE)
  
  # If nfactors not specified, use parallel analysis suggestion
  if (is.null(nfactors)) {
    nfactors <- parallel_result$nfact
    cat(sprintf("\nUsing %d factors suggested by parallel analysis\n", nfactors))
  }
  
  # Run EFA
  cat("\n=== Running EFA ===\n")
  efa_result <- fa(data_subset, nfactors = nfactors, rotate = rotation)
  efa_result <- fa.sort(efa_result)
  ...
  # Return the EFA object invisibly for further use
...
summary(fit2.1, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
summary(fit2.2, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
print(av2)

summary(fit3.1, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
summary(fit3.2, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
print(av3)

summary(fit4.1, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
...
summary(fit6.1, fit.measures = TRUE, standardized = TRUE, rsquare = TRUE)
